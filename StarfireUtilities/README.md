...
### Inline Asset Size
![](./Resources/inline_asset_size.png)

_SInlineAssetSize.h/cpp, AssetSizeSettings.h/cpp_

A custom editor widget that shows some of the data from the Size Map window directly in blueprint and asset editors.

The widget changes color based on the total size of the blueprint (computed similarly to how Size Map works).

By default this widget is not included in toolbars, but can be easily enabled through the associated developer settings.
